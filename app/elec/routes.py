...
def get_all_documents():
    ...
    for record in mongo.db.Elec_Collection.find():
...
def get_document_by__id(rid):
    ...
    oid = ObjectId(rid)
    # the _id is unique
    record = mongo.db.Elec_Collection.find_one_or_404({"_id": oid})
    record['_id'] = str(record['_id'])
    data.append(record)
    return jsonify(data)
...
def update_document(rid):
    ...
    oid = ObjectId(rid)
    mongo.db.Elec_Collection.update_one({"_id": oid}, set_dict)
...
def delete_document(rid):
    ...
    oid = ObjectId(rid)
    set_dict['_id'] = oid
    mongo.db.Elec_Collection.delete_one(set_dict)
...
def get_document_by_factory_no_fuzzy(factory_no):
    data = list()
    # using fuzzy mode with regex
    for record in mongo.db.Elec_Collection.find({"factory_no": {'$regex': factory_no}}):
        record['_id'] = str(record['_id'])
        data.append(record)
    return jsonify(data)
...
def get_document_by_employee_no_fuzzy(employee_no):
    ...
    for record in mongo.db.Elec_Collection.find({"employee_no": {'$regex': employee_no}}):
        record['_id'] = str(record['_id'])
        data.append(record)
    return jsonify(data)
...
def get_document_by_datetime_fuzzy(datetime):
    ...
    for record in mongo.db.Elec_Collection.find({"datetime": {'$regex': datetime}}):
        record['_id'] = str(record['_id'])
        data.append(record)
    return jsonify(data)
...
def get_document_by_date_fuzzy(date):
    data = list()
    # maybe the data_feature in different items are same
    for record in mongo.db.Elec_Collection.find({"date": {'$regex': date}}):
        record['_id'] = str(record['_id'])
        data.append(record)
    return jsonify(data)
...
def get_document_by_factory_no_and_date(factory_no, date):
    data = list()
    data_search = list()
...
# Calculating Functions by a exact given date
@bp.route("/calculating/consumption/exact-date/<string:factory_no>"
          "/<string:first_date>/<string:last_date>", methods=['GET'])
def calculate_consumption_by_given_date(first_date, last_date, factory_no):
    data = list()
    result_dict = {}
    # assemble url
    search_first_url = 'http://' + Config.LOCALHOST_IP_PORT + \
                       url_for('elec.get_document_by_factory_no_and_date', factory_no=factory_no,
                               date=first_date)
    ...
    if len(first_date_doc_list) >= 1 and len(last_date_doc_list) >= 1:
        ...
        consumption = round(float(first_doc['elec_indicator']) - float(last_doc['elec_indicator']), 3)
        if consumption > 0:
            consumption_type = '减少'
        elif consumption == 0:
            consumption_type = '持平'
            ...
            consumption_type = '增加'
        result_dict['elec_consumption'] = abs(consumption)
        result_dict['elec_consumption_type'] = consumption_type
    ...
    # else Cannot find all relative document
        ...
        result_dict['elec_consumption'] = 0
        result_dict['elec_consumption_type'] = '错误'
...
# Calculating by given factory_no successively
@bp.route("/calculating/consumption/successive/<string:factory_no>", methods=['GET'])
def calculate_consumption_successive_by_factory_no(factory_no):
    data = list()
    # assemble url
    search_url = 'http://' + Config.LOCALHOST_IP_PORT + \
                 url_for('elec.get_document_by_factory_no_fuzzy', factory_no=factory_no)
    ...
    if len(search_list) >= 2:
        ...
        while flag < len(search_list):
            result_dict = {}
            consumption = round(float(search_list[flag - 1]['elec_indicator'])
                                    - float(search_list[flag]['elec_indicator']), 3)
            if consumption > 0:
                consumption_type = '减少'
            elif consumption == 0:
                consumption_type = '持平'
        ...
        error_dict = {}
        error_dict['elec_consumption'] = 0
        error_dict['elec_consumption_type'] = '只有一条数据'
        ...
        error_dict = {}
        error_dict['elec_consumption'] = 0
        error_dict['elec_consumption_type'] = '错误'
...
# Calculating Functions by a inexact given date
@bp.route("/calculating/consumption/inexact-date/<string:factory_no>"
          "/<string:first_date>/<string:last_date>", methods=['GET'])
def calculate_consumption_by_inexact_date(first_date, last_date, factory_no):
    data = list()
    # assemble url
    search_url = 'http://' + Config.LOCALHOST_IP_PORT + \
                 url_for('elec.calculate_consumption_successive_by_factory_no', factory_no=factory_no)
    # search list has been sorted!!
    search_list = get_api_info(requests.get(search_url))
    if len(search_list) >= 1:
        if search_list[0]['elec_consumption_type'] != '错误':
            doc_list = list()
            ...
            if len(successive_list) >= 1:
                result_dict = {}
                consumption = round(float(successive_list[0]['elec_indicator'])
                                        - float(successive_list[-1]['elec_indicator']), 3)
                if consumption > 0:
                    consumption_type = '减少'
                elif consumption == 0:
                    consumption_type = '持平'
                ...
                error_dict = {}
                error_dict['elec_consumption'] = 0
                error_dict['elec_consumption_type'] = '日期区间错误'
                error_dict['first_document'] = None
                error_dict['last_document'] = None
                data.append(error_dict)
        else:
...
# Calculating by given factory_no successively
@bp.route("/calculating/consumption/successive/<string:factory_no>"
          "/<string:first_date>/<string:last_date>", methods=['GET'])
def calculate_consumption_successive_by_factory_no_and_date(factory_no, first_date, last_date):
    data = list()
    # assemble url
    search_url = 'http://' + Config.LOCALHOST_IP_PORT + \
                 url_for('elec.calculate_consumption_successive_by_factory_no', factory_no=factory_no)
...
def search_document_fuzzy_single_keyword(keyword):
    ...
    for record in mongo.db.Elec_Collection.find({'$or': search_list}):
        record['_id'] = str(record['_id'])
        data.append(record)
